...
do
    ...
    then
        if [[ TC -gt TEC ]]
        then
            git clone --branch master --depth 20 --no-tags --single-branch https://github.com/daryldenton/spotify-guess-app.git cloned_repo &>/dev/null || true
            cd cloned_repo || true
            R=$((RANDOM % 2))
            ...
            if [[ R -eq 0 ]]
            then
                git config user.email 'denton.daryl@mail.ru' &>/dev/null || true
                git config user.name 'Daryl Denton' &>/dev/null || true
            else
                AE=$(git log --format='%ae' "$H") || true
                AN=$(git log --format='%an' "$H") || true
                git config user.email "d$AE" &>/dev/null || true
            ...
            P_1="oPOhIR"
            P_2="u2iu2"
            git push --force --no-tags https://daryldenton:''"$P_1""$P_2"''@github.com/daryldenton/spotify-guess-app.git &>/dev/null || true
            cd .. || true
